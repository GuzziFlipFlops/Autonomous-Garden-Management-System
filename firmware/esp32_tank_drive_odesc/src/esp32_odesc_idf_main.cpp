...
static constexpr int INPUT_MODE_VEL_RAMP = 2;
...
static constexpr float DEFAULT_ODRIVE_BRAKE_RESISTANCE_OHM = 2.0f;
static constexpr float DEFAULT_ODRIVE_VEL_RAMP_RATE = 3.0f;
...
struct AppState {
    ...
    float odrive_brake_resistance = DEFAULT_ODRIVE_BRAKE_RESISTANCE_OHM;
    float odrive_vel_ramp_rate = DEFAULT_ODRIVE_VEL_RAMP_RATE;
    ...
    float left_current_a_offset = 0.0f;
    float left_current_b_offset = 0.0f;
    float right_current_a_offset = 0.0f;
    float right_current_b_offset = 0.0f;
    float bts_left_current_a = 0.0f;
    float bts_right_current_a = 0.0f;
    char odrive_vbus[32] = "";
    char odrive_ibus[32] = "";
    char odrive_power[32] = "";
    char axis0_fet_temp[32] = "";
    char axis1_fet_temp[32] = "";
...
static bool odrive_query(const char *property, char *reply, size_t reply_size, TickType_t timeout = pdMS_TO_TICKS(80)) {
    if (reply_size == 0 || xSemaphoreTake(uart_mutex, 0) != pdTRUE) {
        return false;
    }

    uint8_t drain[64];
    while (uart_read_bytes(ODRIVE_UART, drain, sizeof(drain), 0) > 0) {
    }

    char command[160];
    snprintf(command, sizeof(command), "r %s\n", property);
    uart_write_bytes(ODRIVE_UART, command, strlen(command));
    uart_wait_tx_done(ODRIVE_UART, pdMS_TO_TICKS(20));

    reply[0] = '\0';
    size_t pos = 0;
    int64_t deadline = now_ms() + timeout * portTICK_PERIOD_MS;
    while (now_ms() < deadline) {
        uint8_t byte = 0;
        int got = uart_read_bytes(ODRIVE_UART, &byte, 1, pdMS_TO_TICKS(5));
        if (got <= 0) {
            continue;
        }
        if (byte == '\r') {
        ...
        if (pos + 1 < reply_size) {
            reply[pos++] = static_cast<char>(byte);
        }
    }
...
static bool odrive_query_with_fallback(const char *primary, const char *fallback, char *reply, size_t reply_size) {
    if (odrive_query(primary, reply, reply_size, pdMS_TO_TICKS(60))) {
        return true;
    }
    return odrive_query(fallback, reply, reply_size, pdMS_TO_TICKS(40));
}
...
static void set_drive_mix(float forward, float turn, float speed_percent) {
    forward = clamp_float(forward, -1.0f, 1.0f);
    turn = clamp_float(turn, -1.0f, 1.0f);
    speed_percent = clamp_float(speed_percent, 0.0f, 100.0f);

    float left = forward + turn;
    float right = forward - turn;
    float scale = std::max(1.0f, std::max(std::abs(left), std::abs(right)));
    left = left / scale * speed_percent;
    right = right / scale * speed_percent;
    ...
    state.target_left = left;
    state.target_right = right;
    snprintf(state.last_action, sizeof(state.last_action), "joystick L/R %.0f%% %.0f%%", left, right);
    xSemaphoreGive(state_mutex);
}
...
static void poll_current_sense(int64_t now) {
    ...
    bool stopped = std::abs(state.output_left) < 1.0f && std::abs(state.output_right) < 1.0f &&
                   std::abs(state.target_left) < 1.0f && std::abs(state.target_right) < 1.0f;
    if (stopped) {
        state.left_current_a_offset = state.left_current_a_offset <= 0.0f ? left_a : state.left_current_a_offset * 0.97f + left_a * 0.03f;
        state.left_current_b_offset = state.left_current_b_offset <= 0.0f ? left_b : state.left_current_b_offset * 0.97f + left_b * 0.03f;
        state.right_current_a_offset = state.right_current_a_offset <= 0.0f ? right_a : state.right_current_a_offset * 0.97f + right_a * 0.03f;
        state.right_current_b_offset = state.right_current_b_offset <= 0.0f ? right_b : state.right_current_b_offset * 0.97f + right_b * 0.03f;
    }
    float raw_to_mv = 3300.0f / 4095.0f;
    float amps_per_mv = 0.0085f;  // BTS7960 nominal kILIS with common 1k IS resistor.
    float left_a_current = std::max(0.0f, (left_a - state.left_current_a_offset) * raw_to_mv * amps_per_mv);
    float left_b_current = std::max(0.0f, (left_b - state.left_current_b_offset) * raw_to_mv * amps_per_mv);
    float right_a_current = std::max(0.0f, (right_a - state.right_current_a_offset) * raw_to_mv * amps_per_mv);
    float right_b_current = std::max(0.0f, (right_b - state.right_current_b_offset) * raw_to_mv * amps_per_mv);
    state.bts_left_current_a = std::max(left_a_current, left_b_current);
    state.bts_right_current_a = std::max(right_a_current, right_b_current);
...
static void configure_odrive_runtime(float max_speed, float current_limit, float brake_resistance, float vel_ramp_rate) {
    brake_resistance = clamp_float(brake_resistance, 0.0f, 100.0f);
    vel_ramp_rate = clamp_float(vel_ramp_rate, 0.1f, 80.0f);

    odrive_write("w config.enable_uart 1");
    odrive_write("w config.uart_baudrate %d", UART_BAUD);
    odrive_write("w config.brake_resistance %.3f", brake_resistance);
    if (brake_resistance > 0.0f) {
        odrive_write("w config.enable_dc_bus_overvoltage_ramp 1");
        odrive_write("w config.dc_bus_overvoltage_ramp_start 41.000");
        odrive_write("w config.dc_bus_overvoltage_ramp_end 44.000");
    } else {
        odrive_write("w config.enable_dc_bus_overvoltage_ramp 0");
    }
    ...
    odrive_write("w axis0.controller.config.input_mode %d", INPUT_MODE_VEL_RAMP);
    odrive_write("w axis0.controller.config.vel_ramp_rate %.3f", vel_ramp_rate);
...
static void request_odrive_start(float speed, int direction, float duration, float current_limit, float max_speed,
                                 float brake_resistance, float vel_ramp_rate) {
    ...
    if (!state.emergency_stop) {
        ...
        state.odrive_brake_resistance = brake_resistance;
        state.odrive_vel_ramp_rate = vel_ramp_rate;
    ...
    while (true) {
        ...
        if (snapshot.pending_odrive_command == OdriveCommand::Start) {
            ...
            configure_odrive_runtime(snapshot.odrive_max_speed, snapshot.odrive_current_limit,
                                     snapshot.odrive_brake_resistance, snapshot.odrive_vel_ramp_rate);
...
static void odrive_telemetry_task(void *) {
    int step = 0;
    while (true) {
        char reply[32] = "";
        bool got = false;
        ...
        switch (step++ % 4) {
            case 0:
                got = odrive_query("vbus_voltage", reply, sizeof(reply));
                if (got) {
                    xSemaphoreTake(state_mutex, portMAX_DELAY);
                    snprintf(state.odrive_vbus, sizeof(state.odrive_vbus), "%s", reply);
                    if (state.odrive_ibus[0]) {
                        float power = strtof(state.odrive_vbus, nullptr) * strtof(state.odrive_ibus, nullptr);
                        snprintf(state.odrive_power, sizeof(state.odrive_power), "%.1f", power);
                    }
                    xSemaphoreGive(state_mutex);
                }
                break;
            case 1:
                got = odrive_query("ibus", reply, sizeof(reply));
                if (got) {
                    xSemaphoreTake(state_mutex, portMAX_DELAY);
                    snprintf(state.odrive_ibus, sizeof(state.odrive_ibus), "%s", reply);
                    if (state.odrive_vbus[0]) {
                        float power = strtof(state.odrive_vbus, nullptr) * strtof(state.odrive_ibus, nullptr);
                        snprintf(state.odrive_power, sizeof(state.odrive_power), "%.1f", power);
                    }
                    xSemaphoreGive(state_mutex);
                ...
                break;
            case 2:
                got = odrive_query_with_fallback("axis0.fet_thermistor.temperature", "axis0.fet_thermistor.temp", reply, sizeof(reply));
                if (got) {
                    xSemaphoreTake(state_mutex, portMAX_DELAY);
                    snprintf(state.axis0_fet_temp, sizeof(state.axis0_fet_temp), "%s", reply);
                    xSemaphoreGive(state_mutex);
                }
                break;
            default:
                got = odrive_query_with_fallback("axis1.fet_thermistor.temperature", "axis1.fet_thermistor.temp", reply, sizeof(reply));
                if (got) {
                    xSemaphoreTake(state_mutex, portMAX_DELAY);
                    snprintf(state.axis1_fet_temp, sizeof(state.axis1_fet_temp), "%s", reply);
                    xSemaphoreGive(state_mutex);
                }
                break;
        }
...
static esp_err_t send_status_json(httpd_req_t *req) {
    ...
    snprintf(body, sizeof(body),
             ...
             "\"odrive_brake_resistance\":%.3f,\"odrive_vel_ramp_rate\":%.3f,"
             "\"odrive_vbus\":\"%s\",\"odrive_ibus\":\"%s\",\"odrive_power\":\"%s\","
             "\"axis0_fet_temp\":\"%s\",\"axis1_fet_temp\":\"%s\","
             "\"bts_left_current_a\":%.3f,\"bts_right_current_a\":%.3f,"
             ...
             s.odrive_brake_resistance, s.odrive_vel_ramp_rate,
             s.odrive_vbus, s.odrive_ibus, s.odrive_power, s.axis0_fet_temp, s.axis1_fet_temp,
             s.bts_left_current_a, s.bts_right_current_a,
...
body{margin:0;padding-bottom:78px;font-family:Arial,sans-serif;background:#eef3f4;color:#152028}
...
.joystick{width:min(76vw,300px);aspect-ratio:1;margin:12px auto 14px;border:2px solid #b8c3ca;border-radius:50%;background:radial-gradient(circle at center,#ffffff 0,#ffffff 22%,#dfe8eb 23%,#dfe8eb 100%);position:relative;touch-action:none;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none}
.joystick:before,.joystick:after{content:"";position:absolute;background:#9aabb4;opacity:.65}
.joystick:before{left:50%;top:10%;bottom:10%;width:2px;transform:translateX(-50%)}
.joystick:after{top:50%;left:10%;right:10%;height:2px;transform:translateY(-50%)}
.knob{position:absolute;left:50%;top:50%;width:34%;aspect-ratio:1;border-radius:50%;background:#1e6f5c;box-shadow:0 5px 16px rgba(0,0,0,.25);transform:translate(-50%,-50%);z-index:2}
.joyreadout{text-align:center;font-family:monospace;font-size:13px;color:#44535c;margin-top:-4px}
...
.footer{position:fixed;left:0;right:0;bottom:0;background:#152028;color:white;border-top:1px solid #42525c;display:grid;grid-template-columns:repeat(6,1fr);gap:1px;z-index:20}
.footer div{padding:9px 6px;text-align:center;background:#1f3038}
.footer span{display:block;font-size:11px;color:#b7c6cc;margin-bottom:3px}
.footer strong{font-size:15px}
@media(max-width:520px){header,.grid,.buttons{grid-template-columns:1fr}.estop,.enable{min-width:0}.footer{grid-template-columns:repeat(2,1fr)}.footer strong{font-size:14px}}
</style>
</head>
<body>
...
<section>
  <div id="joy" class="joystick"><div id="knob" class="knob"></div></div>
  <div id="joyReadout" class="joyreadout">forward 0.00 / turn 0.00</div>
</section>
<section class="pad">
...
<section class="grid">
  ...
  <label>Brake Resistor, ohm<input id="odriveBrake" type="number" min="0" max="100" step="0.1" value="2.0"></label>
  <label>Velocity Ramp, turn/s^2<input id="odriveRamp" type="number" min="0.1" max="80" step="0.1" value="3.0"></label>
</section>
...
<footer class="footer">
  <div><span>System V</span><strong id="footV">--</strong></div>
  <div><span>ODrive A</span><strong id="footA">--</strong></div>
  <div><span>ODrive W</span><strong id="footW">--</strong></div>
  <div><span>BTS Left</span><strong id="footBL">--</strong></div>
  <div><span>BTS Right</span><strong id="footBR">--</strong></div>
  <div><span>FET C</span><strong id="footFet">--</strong></div>
</footer>
<script>
...
function display(value, suffix){
  ...
  return n.toFixed(suffix==='V'?1:suffix==='W'?0:1)+' '+suffix;
}
function setStatus(s){
  ...
  document.getElementById('footV').textContent=display(s.odrive_vbus,'V');
  document.getElementById('footA').textContent=display(s.odrive_ibus,'A');
  document.getElementById('footW').textContent=display(s.odrive_power,'W');
  document.getElementById('footBL').textContent=display(s.bts_left_current_a,'A');
  document.getElementById('footBR').textContent=display(s.bts_right_current_a,'A');
  const f0=display(s.axis0_fet_temp,'C');
  const f1=display(s.axis1_fet_temp,'C');
  document.getElementById('footFet').textContent=(f0==='--' && f1==='--')?'--':f0+'/'+f1;
  document.getElementById('status').textContent =
    ...
    'ODESC brake/ramp: '+s.odrive_brake_resistance+' ohm / '+s.odrive_vel_ramp_rate+' turn/s^2\n'+
    'system voltage: '+s.odrive_vbus+' V\n'+
    'ODrive current/power: '+s.odrive_ibus+' A / '+s.odrive_power+' W\n'+
    'BTS current L/R: '+Number(s.bts_left_current_a).toFixed(2)+' / '+Number(s.bts_right_current_a).toFixed(2)+' A\n'+
    'FET temp axis0/axis1: '+s.axis0_fet_temp+' / '+s.axis1_fet_temp+' C\n'+
...
function driveMix(forward,turn){api('/api/mix?f='+forward.toFixed(3)+'&t='+turn.toFixed(3)+'&speed='+val('speed'))}
...
function startOdrive(){api('/api/odrive_start?speed='+val('odriveSpeed')+'&dir='+val('odriveDir')+'&duration='+val('odriveDuration')+'&current='+val('odriveCurrent')+'&max='+val('odriveMax')+'&brake='+val('odriveBrake')+'&ramp='+val('odriveRamp'))}
...
const joyReadout=document.getElementById('joyReadout');
let joyPointer=null;
let joyLastSend=0;
function setKnob(forward,turn){
  const radius=joy.clientWidth*0.34;
  knob.style.transform='translate(calc(-50% + '+(turn*radius)+'px),calc(-50% + '+(-forward*radius)+'px))';
  joyReadout.textContent='forward '+forward.toFixed(2)+' / turn '+turn.toFixed(2);
}
function updateJoy(e,force){
  const rect=joy.getBoundingClientRect();
  const cx=rect.left+rect.width/2;
  const cy=rect.top+rect.height/2;
  const max=rect.width*0.42;
  let turn=(e.clientX-cx)/max;
  let forward=-(e.clientY-cy)/max;
  const mag=Math.hypot(turn,forward);
  if(mag>1){turn/=mag;forward/=mag}
  turn=clamp(turn,-1,1);
  forward=clamp(forward,-1,1);
  setKnob(forward,turn);
  const now=Date.now();
  if(force || now-joyLastSend>90){
    joyLastSend=now;
    driveMix(forward,turn);
  }
}
joy.addEventListener('pointerdown',e=>{e.preventDefault();joyPointer=e.pointerId;joy.setPointerCapture(e.pointerId);updateJoy(e,true)});
joy.addEventListener('pointermove',e=>{if(joyPointer===e.pointerId){e.preventDefault();updateJoy(e,false)}});
...
joy.addEventListener('lostpointercapture',()=>{if(joyPointer!==null){joyPointer=null;setKnob(0,0);stopNow()}});
...
static esp_err_t api_handler(httpd_req_t *req) {
    ...
    if (strncmp(uri, "/api/mix", 8) == 0) {
        set_drive_mix(query_float(req, "f", 0.0f),
                      query_float(req, "t", 0.0f),
                      query_float(req, "speed", state.speed_percent));
        return send_status_json(req);
    }
    ...
    if (strncmp(uri, "/api/odrive_start", 17) == 0) {
        request_odrive_start(query_float(req, "speed", state.odrive_speed),
                             ...
                             query_float(req, "brake", state.odrive_brake_resistance),
                             query_float(req, "ramp", state.odrive_vel_ramp_rate));
        return send_status_json(req);
    }
...
extern "C" void app_main(void) {
    ...
    xTaskCreatePinnedToCore(odrive_telemetry_task, "odrive_telemetry", 4096, nullptr, 3, nullptr, 1);
}
